...
#save as .txt
write.table(sub_stats_for_MRI, file = "sub_mem_stats_jan27.txt", sep = "\t",
          row.names = F)
```
...
RS_age_plot <- ggplot(RS_means, aes(x = age, y = mean_RS)) +
  ...
  ylab("Mean Repetition Suppression") +
  xlab("Age") +
  kate.theme
RS_age_plot
```

```{r RS box plots, fig.height = 4, fig.width = 5}
temp <- RS_data_all %>%
  filter(phc_decrease > phc_outlier_min) %>%
  filter(phc_decrease < phc_outlier_max) %>% 
  pivot_longer(cols = c(phc1, phc5), names_to = "repetition", names_prefix = "phc", values_to = "activity")

RS_age_group_plot <- ggplot(temp, aes(x = ageGroup, color = repetition, y = activity)) +
  geom_boxplot(aes(fill = repetition), alpha = .3) +
  scale_fill_manual(values = c(color1, color3), name = "Repetition") +
  scale_color_manual(values = c(color1, color3), name = "Repetition") +
  ylab("Neural Activity") +
  xlab("Age Group") +
  kate.theme
RS_age_group_plot
```
