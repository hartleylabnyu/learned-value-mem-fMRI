...
temp <- full_join(memData, RS_estimates, by = c("sub", "stim", "block")) %>%
  filter(freqCond == 5) %>%
...
#display results
model.1.2
# There is a relationship between age^2 and repetition suppression.
...
# run model
model.3 <- mixed(memAcc ~ phc_decrease_scaled * IQ_scaled * (age_scaled + age_squared_scaled) + 
                   (phc_decrease_scaled ||sub), 
                 data = model3_data,
                 family = "binomial",
                 method = "LRT",
...
# run model
model.4 <- mixed(memAcc ~ (age_scaled + age_squared_scaled) * IQ_scaled * phc_decrease_scaled * freq_report_scaled  + (phc_decrease_scaled * freq_report_scaled|sub), 
                 data = model4_data,
                 family = "binomial",
                 method = "LRT",
                 control = glmerControl(optCtrl = list(maxfun=1e6), optimizer = "bobyqa"), 
